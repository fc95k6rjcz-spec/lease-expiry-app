...
export function reconcileBoard(scanned = [], existing = [], { detectDepartures = true } = {}) {
  ...
  for (const l of existing) {
    ...
    if (!n) continue;
    if (!known.has(n)) known.set(n, { name, floor: floorOf(l), seen: false });
  }
  ...
  for (const s of scanned) {
    ...
    if (fNew && fOld && fNew !== fOld) {
      moved.push({ tenant: name, from: hit.floor, to: s.floor || '' });
    } else {
      matched.push({ tenant: name, floor: s.floor || hit.floor || '' });
    }
  }
  ...
  if (detectDepartures) {
    for (const v of known.values()) {
      if (!v.seen) departed.push({ tenant: v.name, floor: v.floor });
    }
  }
