...
// One rule for "don't show this as a live target", used across dashboard, lead finder, targets.
export function isSuppressed(t) {
  if (!t) return false;
  if (t.relationship === 'Competitor' || t.relationship === 'Lost') return true;
  if (t.prospect_status === 'moved' || t.prospect_status === 'done') return true;
  if (DISMISSED.includes(t.occupier_status)) return true;
  return false;
}
...
function enrich(o, statusOf) {
  ...
  const dismissed = operator || DISMISSED.includes(status)
    || ['Competitor', 'Lost'].includes(o.tenant_obj?.relationship)
    || ['moved', 'done'].includes(o.tenant_obj?.prospect_status || '');
